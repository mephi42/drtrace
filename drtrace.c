...
#define INSERT(instr) \
  ...
  // *current_reg = bb_id
  u32 = (uintptr_t)tag & 0xFFFFFFFFu;
  INSERT(
      INSTR_CREATE_mov_st(drcontext,
                          OPND_CREATE_MEMPTR(current_reg, 0),
                          opnd_create_immed_int(u32, OPSZ_4)));
#ifdef X64
  u32 = (uintptr_t)tag >> 32;
  INSERT(
      INSTR_CREATE_mov_st(drcontext,
...
dr_signal_action_t handle_signal(void* drcontext, dr_siginfo_t* siginfo) {
  ...
  if(siginfo->sig == SIGSEGV) {
    ...
    tb_flush(tb);
    dr_fprintf(STDERR, "%p %p\n", siginfo->access_address, tb_end(tb));
    if(siginfo->access_address == tb_end(tb)) {
      dr_fprintf(STDERR, "zzz\n");
    }
  }
